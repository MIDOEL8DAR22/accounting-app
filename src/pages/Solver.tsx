...
export function Solver() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-extrabold text-slate-800 dark:text-slate-100 sm:text-2xl">🧠 حل أي سؤال محاسبي</h1>
      ...
      <Card>
        <label className="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">✍️ اكتب العملية:</label>
        <textarea
          value={input}
      ...
      <div>
        <div className="mb-2 text-xs font-bold text-slate-500 dark:text-slate-400">💡 جرّب أمثلة جاهزة:</div>
      ...
      {noMatch && (
        <Card className="border-amber-200 bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/10">
          ...
          <div className="mt-3 space-y-2 text-xs text-amber-600 dark:text-amber-500">
            <div className="font-bold">💡 أمثلة على عمليات أقدر أحلها:</div>
      ...
      {result && (
        <div className="space-y-4 animate-fade-in-up">
          {result.pattern.assumption && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
              ⚠️ افتراض: {result.pattern.assumption}
            </div>
          )}
